...
import { loadTaskPolicy } from "../policy/task-policy.ts";
import { mutonHome } from "../store/fs.ts";

/** Built-in reflection prompt. Always used as the base; extras are appended. */
...
- Example top-level folders when they fit: schema, encoding, lookup, preference, procedure, api.
- Add a second segment when useful (e.g. schema/joins, api/auth, deploy/k8s).
...
- Skip: one-off plans, full transcripts, secrets/credentials, generic advice, reminders the task already states, ephemeral debugging chatter.
...
/**
 * Default prompt, plus task-policy ## Reflection (MUTON_TASK_POLICY), plus
 * the first extra REFLECTION.md found (project, then home).
 */
export function loadReflectionPrompt(opts?: { cwd?: string; home?: string }): string {
  const cwd = opts?.cwd ?? process.cwd();
  const home = opts?.home ?? mutonHome();
  const parts = [DEFAULT_REFLECTION_PROMPT];

  const policy = loadTaskPolicy();
  if (policy.reflectionExtra) {
    parts.push(policy.reflectionExtra);
  }

  const extraMd = readExtraPrompt(cwd, home);
  if (extraMd) parts.push(extraMd);
